...
sumoCmd = [sumolib.checkBinary('sumo'), '-c', 'osm.sumocfg', '--log', 'simulation_main.log','--random']
...
<routes xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="http://sumo.dlr.de/xsd/routes_file.xsd">
    ...
    <routeDistribution id="rd1">
        <route id="r_0" edges="1024002010 1024002009 41183908#1" probability="0.2"/>
        <route id="r_1" edges="41183912#0 41183912#1 41183912#2 41183912#3 -473338926#0 -1024002010" probability="0.2"/>
        <route id="r_2" edges="-41183908#2 -473338925#0 -41183912#2 -41183912#1 -41183912#0" probability="0.2"/>
        <route id="r_3" edges="-41183908#2 224109042#1 1024002008#0 1024002008#1" probability="0.2"/>
        <route id="r_4" edges="-1024002008#1 -1024002008#0 -224109042#1 -473338926#0 -1024002010" probability="0.2"/>
    </routeDistribution>
...
"""
    with open("osm.passenger.rou.xml", "w") as xml_file:
...
def run_simulations_only_collisions():
    ...
    while no_collision_count < 5:
        ...
        if fcd_data['collision_occurred'].sum() == 0:
            ...
            csv_filename = f"VECM_analysis/simulation_no_collision_{no_collision_count}_dataset.csv"
            #ego_df.to_csv(csv_filename, index=False)
            fcd_data.to_csv(f'VECM_analysis/no_collision{no_collision_count}.csv', index=False)
    ...
    while collision_count < 10:
        ...
        if fcd_data_col['collision_occurred'].sum() > 0:
            ...
            csv_filename = f"VECM_analysis/simulation_collision_{collision_count}_dataset.csv"
            #ego_df.to_csv(csv_filename, index=False)
            fcd_data_col.to_csv(f'VECM_analysis/collision{collision_count}.csv', index=False)
